function Export-SPSUpdateProgressReport {
    ...
    $footerNote = if ($effectiveRefresh -gt 0) {
        'It is assembled from the shared status store and refreshes itself while patching is in progress.'
    }
    else {
        'It is assembled from the shared status store and reflects the completed campaign.'
    }
    ...
    $sections +
    "<div class=`"footer`">Generated by SPSUpdate $encVersion. $footerNote</div>" +
    '</body></html>'
